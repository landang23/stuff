...
def generate_detailed_transactions(profiles, products):
    ...
    for _, row in profiles.iterrows():
        num_txns = random.randint(1, 25) # Generates between 1 & 15 transactions for each customer_id 
        if num_txns == 0:
            continue
        ...
        for i in range(num_txns):
          ...
          txn_date = pd.to_datetime("2025-09-30") - pd.to_timedelta(np.random.randint(1, 365), unit='D')
